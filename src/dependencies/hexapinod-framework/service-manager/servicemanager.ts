...
class ServiceManagerTool {
  ...
  protected async initializeServicesClass (_directory: string): Promise<boolean> {
    this.services = [];
    const list = fs.readdirSync(decodeURI(new URL('.', import.meta.url).pathname) + '../../../core/' + _directory);
    for (let i = 0; i < list.length; i++) {
      if (!list[i].endsWith('.service.ts') && !list[i].endsWith('.service.js')) {
        continue;
      }
      logger.info('load service ' + list[i]);
      const service = await import(decodeURI(new URL('.', import.meta.url).pathname) + '../../../core/' + _directory + list[i]);
