...
export class ValidatorService extends Service {
    ...
    public async loadValidatorSchemas(): Promise<void> {
      ...
      const bundles = fs.readdirSync(decodeURI(new URL('.', import.meta.url).pathname) + '../../', { withFileTypes: true });
      for (let i = 0; i < bundles.length; i++) {
        if (bundles[i].isDirectory()) {
          logger.info('load bundle ' + bundles[i].name + ' validators' );
          if (!fs.existsSync(decodeURI(new URL('.', import.meta.url).pathname) + '../../' + bundles[i].name + '/services/validator')) {
            logger.warn('path does\'t exists, no validator available for bundle: ' + decodeURI(new URL('.', import.meta.url).pathname) + '../../' + bundles[i].name + '/services/validator');
            continue;
          }
          const list = fs.readdirSync(decodeURI(new URL('.', import.meta.url).pathname) + '../../' + bundles[i].name + '/services/validator');
          for (let j = 0; j < list.length; j++) {
            if (!list[j].endsWith('.schema.ts') && !list[j].endsWith('.schema.js')) {
              continue;
            }
            try {
              const schema = await import(decodeURI(new URL('.', import.meta.url).pathname) + '../../' + bundles[i].name + '/services/validator/' + list[j]);
