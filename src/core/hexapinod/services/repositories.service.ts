...
export class RepositoriesService extends Service {
  ...
  async getRepositoryByName (_className:string): Promise<IRepository> {
    ...
    if (!fs.existsSync(new URL('.', import.meta.url).pathname + '../../../infrastructure/repositories/' + (process.env.SERVICE_REPOSITORY_FOLDERS ? process.env.SERVICE_REPOSITORY_FOLDERS + '/' : '') + reponame + '.ts')) {
    ...
    const paramRepo = await import(new URL('.', import.meta.url).pathname + '../../../infrastructure/repositories/' + (process.env.SERVICE_REPOSITORY_FOLDERS ? process.env.SERVICE_REPOSITORY_FOLDERS + '/': '') + reponame);
